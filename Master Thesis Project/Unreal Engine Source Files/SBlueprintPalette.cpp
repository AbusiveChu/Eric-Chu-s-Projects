...
//	TSharedRef<SWidget> AssetDiscoveryIndicator = EditorWidgetsModule.CreateAssetDiscoveryIndicator(EAssetDiscoveryIndicatorScaleMode::Scale_Vertical);
//
//	float FavoritesHeightRatio = 0.9f;
//	GConfig->GetFloat(*BlueprintPalette::ConfigSection, *BlueprintPalette::FavoritesHeightConfigKey, FavoritesHeightRatio, GEditorPerProjectIni);
//	float LibraryHeightRatio = 0.1f;
//	GConfig->GetFloat(*BlueprintPalette::ConfigSection, *BlueprintAlternative::LibraryHeightConfigKey, LibraryHeightRatio, GEditorPerProjectIni);
//
//	bool bUseLegacyLayout = false;
//	GConfig->GetBool(*BlueprintPalette::ConfigSection, TEXT("bUseLegacyLayout"), bUseLegacyLayout, GEditorIni);
//
...
//		this->ChildSlot
//			[
//				SAssignNew(LibraryWrapper, SBlueprintAlternativesLibrary, InBlueprintEditor)
//				.UseLegacyLayout(bUseLegacyLayout)
//			];
//	}
//	else
...
//		this->ChildSlot
//			[
//				SAssignNew(AlternativeListView,SListView<TSharedRef<FAlternativeData>>)
...
//		//this->ChildSlot
//		//	[
//		//		SAssignNew(AlternativesSplitter, SSplitter)
//		//		.Orientation(Orient_Vertical)
//		//		.OnSplitterFinishedResizing(this, &SBlueprintAlternatives::OnSplitterResized)
//		//		.AddMetaData<FTagMetaData>(FTagMetaData(TEXT("FullBlueprintPalette")))
//		//
//		//// Alternatives Slot
//		//+ SSplitter::Slot()
//		//	.Value(LibraryHeightRatio)
//		//	[
//		//		SNew(SBlueprintAlternativesLibrary, InBlueprintEditor)
//		//		.AddMetaData<FTagMetaData>(FTagMetaData(TEXT("BlueprintAlternativesLibrary")))
//		//	]
//		//	];
//	} 
//}
//
//void SBlueprintAlternatives::OnSplitterResized() const
//{
//	FChildren const* const SplitterChildren = AlternativesSplitter->GetChildren();
//	for (int32 SlotIndex = 0; SlotIndex < SplitterChildren->Num(); ++SlotIndex)
//	{
//		SSplitter::FSlot const& SplitterSlot = AlternativesSplitter->SlotAt(SlotIndex);
//
//		GConfig->SetFloat(*BlueprintAlternative::ConfigSection, *BlueprintAlternative::LibraryHeightConfigKey, SplitterSlot.SizeValue.Get(), GEditorPerProjectIni);
//	}
//}
//
...
//
/*******************************************************************************
* SBlueprintAlternativesItem Public Interface
*******************************************************************************/

//------------------------------------------------------------------------------
BEGIN_SLATE_FUNCTION_BUILD_OPTIMIZATION
void SBlueprintAlternativesItem::Construct(const FArguments& InArgs, FCreateWidgetForActionData* const InCreateData, TWeakPtr<FBlueprintEditor> InBlueprintEditor)
{
	Construct(InArgs, InCreateData, InBlueprintEditor.Pin()->GetBlueprintObj(), InBlueprintEditor);
}

void SBlueprintAlternativesItem::Construct(const FArguments& InArgs, FCreateWidgetForActionData* const InCreateData, UBlueprint* InBlueprint)
{
	Construct(InArgs, InCreateData, InBlueprint, TWeakPtr<FBlueprintEditor>());
}

void SBlueprintAlternativesItem::Construct(const FArguments& InArgs, FCreateWidgetForActionData* const InCreateData, UBlueprint* InBlueprint, TWeakPtr<FBlueprintEditor> InBlueprintEditor)
{
	check(InCreateData->Action.IsValid());
	check(InBlueprint);
	...
	FSlateColor        IconColor = FSlateColor::UseForeground();
	FSlateColor        SecondaryIconColor = FSlateColor::UseForeground();
	FText			   IconToolTip = FText::FromString(TEXT("Alternative"));
	FString			   IconDocLink, IconDocExcerpt;
	GetPaletteItemIcon(GraphAction, Blueprint, IconBrush, IconColor, IconToolTip, IconDocLink, IconDocExcerpt, SecondaryBrush, SecondaryIconColor);
	TSharedRef<SWidget> IconWidget = CreateIconWidget(IconToolTip, IconBrush, IconColor, IconDocLink, IconDocExcerpt, SecondaryBrush, SecondaryIconColor);
	IconWidget->SetEnabled(!bIsFullyReadOnly);
	...
	// Setup a meta tag for this node
	FTutorialMetaData TagMeta("AlternativesItem");
	if (ActionPtr.IsValid())
	{
		TagMeta.Tag = *FString::Printf(TEXT("AlternativesItem,%s,%d"), *GraphAction->GetMenuDescription().ToString(), GraphAction->GetSectionID());
		TagMeta.FriendlyName = GraphAction->GetMenuDescription().ToString();
	}
	// construct the text widget
	FSlateFontInfo NameFont = FSlateFontInfo(FPaths::EngineContentDir() / TEXT("Slate/Fonts/Roboto-Regular.ttf"), 10);
	bool bIsReadOnly = false; //@Alternatives: Always writable for now may be subject to change
	TSharedRef<SWidget> NameSlotWidget = CreateTextSlotWidget(NameFont, InCreateData, bIsReadOnly);
