...
void FBlueprintEditor::CreateDefaultTabContents(const TArray<UBlueprint*>& InBlueprints)
{
	...
	if (InBlueprint)
	{
		...
		this->BlueprintAlternatives=
			SNew(SBlueprintAlternatives, SharedThis(this))
				.IsEnabled(this, &FBlueprintEditor::IsFocusedGraphEditable);
	}
...
void FBlueprintEditor::CreateDefaultCommands()
{
	...
	// My Commands Blueprint Alternatives
	ToolkitCommands->MapAction( FBlueprintEditorCommands::Get().SaveAlternative,
		FExecuteAction::CreateSP(this, &FBlueprintEditor::SaveAlternative )
		);

	ToolkitCommands->MapAction( FBlueprintEditorCommands::Get().LoadAlternative,
		FExecuteAction::CreateSP(this, &FBlueprintEditor::LoadAlternative)
	);

	ToolkitCommands->MapAction( FBlueprintEditorCommands::Get().OpenBlueprintAlternatives,
		FExecuteAction::CreateSP(this, &FBlueprintEditor::OpenBlueprintAlternatives)
	);
...
void FBlueprintEditor::DummyFunction(int dummy)
{
	int i = 0;
	return;
}

// Protected Alternative Functions

void FBlueprintEditor::SaveAlternative()
{
	SelectAllNodes();
	CopySelectedNodes();

	Alternatives[AltCount] = CurrentNodes;

	// Save 
}

void FBlueprintEditor::LoadAlternative() 
{
	SelectAllNodes();
	DeleteSelectedDuplicatableNodes();

	FPlatformMisc::ClipboardCopy(*Alternatives[0]);
	PasteNodes();

	// Load
}

void FBlueprintEditor::OpenBlueprintAlternatives()
{
	//Open Blueprint Alternative tab
	TabManager->InvokeTab(FBlueprintEditorTabs::BlueprintAlternativesID);

}

// Public Alternative Functions

void FBlueprintEditor::SaveAlternative(bool check)
{
	if (check)
	{
		SelectAllNodes();
...
void FBlueprintEditor::LoadAlternative(bool check)
{
	if (check)
	{
		SelectAllNodes();
