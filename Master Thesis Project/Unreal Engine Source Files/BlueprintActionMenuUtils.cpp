...
namespace BlueprintActionMenuUtilsImpl
{
	...
	static void AddAlternativesSection(FBlueprintActionFilter const& MainMenuFilter, FBlueprintActionMenuBuilder& MenuOut);
}
...
//------------------------------------------------------------------------------
static void BlueprintActionMenuUtilsImpl::AddAlternativesSection(FBlueprintActionFilter const& MainMenuFilter, FBlueprintActionMenuBuilder& MenuOut)
{
	const UBlueprintEditorSettings* BlueprintSettings = GetDefault<UBlueprintEditorSettings>();
	if (BlueprintSettings->bShowContextualFavorites)
	{
		...
		uint32 SectionFlags = 0x00;
		FText  SectionHeading = LOCTEXT("ContextMenuAlternativesTitle", "Alternatives");
...
//------------------------------------------------------------------------------
void FBlueprintActionMenuUtils::MakeAlternativesMenu(FBlueprintActionContext const& Context, FBlueprintActionMenuBuilder& MenuOut)
{
	MenuOut.Empty();

	FBlueprintActionFilter MenuFilter;
	MenuFilter.Context = Context;
	// @ Temporary, add a test later to check to see if an alternative is repeated
	//MenuFilter.AddRejectionTest(FBlueprintActionFilter::FRejectionTestDelegate::CreateStatic(BlueprintActionMenuUtilsImpl::IsNonFavoritedAction));
