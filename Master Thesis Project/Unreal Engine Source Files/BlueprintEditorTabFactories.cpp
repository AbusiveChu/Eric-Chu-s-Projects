...
FBlueprintAlternativeSummoner::FBlueprintAlternativeSummoner(TSharedPtr<class FAssetEditorToolkit> InHostingApp)
	: FWorkflowTabFactory(FBlueprintEditorTabs::BlueprintAlternativesID, InHostingApp)
{
	TabLabel = LOCTEXT("BlueprintAlternativeTabTitle", "Blueprint Alternative");
	TabIcon = FSlateIcon(FEditorStyle::GetStyleSetName(), "ClassIcon.BlueprintCore");

	bIsSingleton = true;

	ViewMenuDescription = LOCTEXT("BlueprintAlternativeView", "Blueprint Alternative");
	ViewMenuTooltip = LOCTEXT("BlueprintAlternativeView_ToolTip", "Show alternatives for the current blueprint");
}

TSharedRef<SWidget> FBlueprintAlternativeSummoner::CreateTabBody(const FWorkflowTabSpawnInfo& Info) const
{
	TSharedPtr<FBlueprintEditor> BlueprintEditorPtr = StaticCastSharedPtr<FBlueprintEditor>(HostingApp.Pin());

	return BlueprintEditorPtr->GetBlueprintAlternatives();
}
